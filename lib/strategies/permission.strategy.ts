...
import { 
  ...
  FileText, Heart, Lock, PillBottle, Shield, Settings,
...
const PERMISSION_MENU_MAP: Record<string, MenuItem> = {
  ...
  // Appointments
  'APPOINTMENTS_VIEW': { icon: Calendar, label: "Appointments", path: "/appointments", permission: 'APPOINTMENTS_VIEW' },
  'APPOINTMENTS_CREATE': { icon: Calendar, label: "Appointments", path: "/appointments", permission: 'APPOINTMENTS_CREATE' },
  'APPOINTMENTS_UPDATE': { icon: Calendar, label: "Appointments", path: "/appointments", permission: 'APPOINTMENTS_UPDATE' },
  'APPOINTMENTS_CANCEL': { icon: Calendar, label: "Appointments", path: "/appointments", permission: 'APPOINTMENTS_CANCEL' },
  
  // Medical Records
  'MEDICAL_RECORDS_READ': { icon: ClipboardList, label: "Clinical Records", path: "/clinical", permission: 'MEDICAL_RECORDS_READ' },
  'MEDICAL_RECORDS_CREATE': { icon: ClipboardList, label: "Clinical Records", path: "/clinical", permission: 'MEDICAL_RECORDS_CREATE' },
  'MEDICAL_RECORDS_UPDATE': { icon: ClipboardList, label: "Clinical Records", path: "/clinical", permission: 'MEDICAL_RECORDS_UPDATE' },
  'MEDICAL_RECORDS_DELETE': { icon: ClipboardList, label: "Clinical Records", path: "/clinical", permission: 'MEDICAL_RECORDS_DELETE' },
  
  // Billing
  'BILLING_VIEW_INVOICES': { icon: BarChart3, label: "Financial", path: "/financial", permission: 'BILLING_VIEW_INVOICES' },
  'BILLING_CREATE_INVOICES': { icon: BarChart3, label: "Financial", path: "/financial", permission: 'BILLING_CREATE_INVOICES' },
  'BILLING_PROCESS_PAYMENTS': { icon: BarChart3, label: "Financial", path: "/financial", permission: 'BILLING_PROCESS_PAYMENTS' },
  
  // System Management
  'SYSTEM_MANAGE_ROLES': { icon: Lock, label: "Security", path: "/security", permission: 'SYSTEM_MANAGE_ROLES' },
  'SYSTEM_HOSPITAL_SETTINGS': { icon: Settings, label: "Settings", path: "/settings", permission: 'SYSTEM_HOSPITAL_SETTINGS' },
  'USERS_MANAGE_PERMISSIONS': { icon: Shield, label: "Permissions", path: "/permissions", permission: 'USERS_MANAGE_PERMISSIONS' },
...
export class PermissionStrategy {
  static getMenuItems(userPermissions: Permission[]): MenuItem[] {
    ...
    if (permissionNames.includes('SYSTEM_HOSPITAL_SETTINGS')) {
      menuItems.push({ icon: Settings, label: "Settings", path: "/settings", permission: 'SYSTEM_HOSPITAL_SETTINGS' });
    }
    
    if (permissionNames.includes('USERS_MANAGE_PERMISSIONS')) {
      menuItems.push({ icon: Shield, label: "Permissions", path: "/permissions", permission: 'USERS_MANAGE_PERMISSIONS' });
    }
