    ...
    const getClinicalStats = () => {
        if (user?.role === 'doctor') {
            return [
                {
                    label: 'My Patients',
                    value: user.assignedPatients?.length?.toString() || '0',
                    icon: Activity,
                    color: 'text-blue-600'
                },
                {label: 'Critical Cases', value: '1', icon: AlertTriangle, color: 'text-red-600'},
                {label: 'Pending Lab Results', value: '3', icon: TestTube, color: 'text-purple-600'},
                {label: 'Procedures Today', value: '2', icon: Stethoscope, color: 'text-green-600'}
            ];
        }

        // Admin/Nurse stats
        return [
            {label: 'Active Patients', value: '156', icon: Activity, color: 'text-blue-600'},
            {label: 'Critical Cases', value: '8', icon: AlertTriangle, color: 'text-red-600'},
            {label: 'Pending Lab Results', value: '23', icon: TestTube, color: 'text-purple-600'},
            {label: 'Procedures Today', value: '12', icon: Stethoscope, color: 'text-green-600'}
        ];
    };
    ...
    const getPatientOptions = () => {
        if (user?.role === 'doctor') {
            ...
            const assignedPatients = [
                {value: 'john-smith', label: 'John Smith (HC001234)', doctorId: '3'},
                {value: 'emma-davis', label: 'Emma Davis (HC001235)', doctorId: '2'},
                {value: 'michael-johnson', label: 'Michael Johnson (HC001236)', doctorId: '4'}
            ];

            return assignedPatients.filter(patient => {
                if (user.id === '2') return patient.doctorId === '2'; // Dr. Michael Chen
                if (user.id === '3') return patient.doctorId === '3'; // Dr. Sarah Johnson
                if (user.id === '4') return patient.doctorId === '4'; // Dr. Emily Rodriguez
                return false;
            });
        }

        // Admin/Nurse can see all patients
        return [
            {value: 'john-smith', label: 'John Smith (HC001234)', doctorId: '3'},
            {value: 'emma-davis', label: 'Emma Davis (HC001235)', doctorId: '2'},
            {value: 'michael-johnson', label: 'Michael Johnson (HC001236)', doctorId: '4'}
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-semibold text-foreground">
                        {user?.role === 'patient' ? 'My Health Records' :
                            user?.role === 'doctor' ? 'My Patients - Clinical Interface' : 'Clinical Interface'}
                    </h2>
            ...
            <Tabs defaultValue="vitals" className="w-full">
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    <CardTitle>
                                        {user?.role === 'patient' ? 'My Current Vital Signs' : 'Current Vital Signs'}
                                    </CardTitle>
                                    <CardDescription>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {medicalData?.vitals?.[0] ? (
                                    <>
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xl font-semibold">{medicalData.vitals[0].temperature}°F</p>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xl font-semibold">{medicalData.vitals[0].heartRate} bpm</p>
                                                <Badge className="bg-green-100 text-green-800">Normal</Badge>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xl font-semibold">{medicalData.vitals[0].bloodPressure}</p>
                                                <Badge className="bg-green-100 text-green-800">Normal</Badge>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xl font-semibold">{medicalData.vitals[0].oxygenSaturation}%</p>
                                                <Badge className="bg-green-100 text-green-800">Normal</Badge>
                                            </div>
                                        </div>
                                    </>
                                    ...
                                    <div className="col-span-4 text-center text-muted-foreground">
                                        {loading ? 'Loading vitals...' : 'No vital signs data available'}
                                    </div>
                                )}
                            </div>
                            ...
                            {medicalData?.vitals?.[0] && (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6 pt-6 border-t">
                                    <div>
                                        <Label>Weight</Label>
                                        <p className="text-lg font-medium">{medicalData.vitals[0].weight} kg</p>
                                    </div>
                                    <div>
                                        <Label>Height</Label>
                                        <p className="text-lg font-medium">{medicalData.vitals[0].height} cm</p>
                                    </div>
                                    <div>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">Stable within normal range</p>
                                </div>
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">Good cardiovascular health</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    <CardTitle>
                                        {user?.role === 'patient' ? 'My Laboratory Results' : 'Laboratory Results'}
                                    </CardTitle>
                                    <CardDescription>
                            ...
                            <div className="space-y-4">
                                {medicalData?.labResults?.length > 0 ? (
                                        ...
                                        <div key={result.id}
                                            ...
                                            <div className="flex items-center space-x-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-muted-foreground">Result: {result.testResults}</p>
                                                    <p className="text-sm text-muted-foreground">Date: {result.testDate}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <Badge className={result.abnormalFlag === 'true' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}>
                                                    {result.abnormalFlag === 'true' ? 'abnormal' : 'normal'}
                                                </Badge>
                                    ...
                                    <div className="text-center text-muted-foreground">
                                        {loading ? 'Loading lab results...' : 'No lab results available'}
                                    </div>
                                )}
                            </div>
                        </CardContent>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <Textarea id="chief-complaint" placeholder="Patient's primary concern..."/>
                                    </div>
                                    <div>
                                        <Label htmlFor="physical-exam">Physical Examination</Label>
                                        <Textarea id="physical-exam" placeholder="Physical examination findings..."/>
                                    </div>
                                    <div>
                ...
                <TabsContent value="procedures">
                    <Card>
                        <CardHeader>
                            <CardTitle>
                                {user?.role === 'patient' ? 'My Procedures & Treatments' : 'Procedures & Treatments'}
                            </CardTitle>
                            <CardDescription>
                                {user?.role === 'patient'
                ...
                <TabsContent value="imaging">
                    <Card>
                        <CardHeader>
                            <CardTitle>
                                {user?.role === 'patient' ? 'My Medical Imaging' : 'Medical Imaging'}
                            </CardTitle>
                            <CardDescription>
                                {user?.role === 'patient'
