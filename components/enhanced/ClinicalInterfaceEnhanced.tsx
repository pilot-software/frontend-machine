...
export function ClinicalInterfaceEnhanced() {
    ...
    const getFilteredData = <T extends { patientId: string; patientName: string }>(data: T[]) => {
        let filtered = data;

        if (user?.role === 'doctor') {
            // Filter to only show data for doctor's assigned patients
            filtered = filtered.filter(item => user.assignedPatients?.includes(item.patientId));
        } else if (user?.role === 'patient') {
            // Filter to only show patient's own data
            filtered = filtered.filter(item => item.patientName.includes(user.name || ''));
        }
        ...
        if (searchTerm) {
            filtered = filtered.filter(item => {
                ...
                if ('testName' in item && typeof (item as unknown as LabResult).testName === 'string' && (item as unknown as LabResult).testName.toLowerCase().includes(searchLower)) return true;
                if ('diagnosisName' in item && typeof (item as unknown as Diagnosis).diagnosisName === 'string' && (item as unknown as Diagnosis).diagnosisName.toLowerCase().includes(searchLower)) return true;
                if ('planName' in item && typeof (item as unknown as TreatmentPlan).planName === 'string' && (item as unknown as TreatmentPlan).planName.toLowerCase().includes(searchLower)) return true;
    ...
    const clinicalStats = [
        ...
        {
            label: 'Pending Labs',
            value: mockLabResults.filter(lab => lab.status === 'In Progress' || lab.status === 'Ordered').length.toString(),
            icon: TestTube,
        ...
        },
        {
            ...
            value: mockTreatmentPlans.filter(plan => plan.status === 'Active').length.toString(),
            icon: FileText,
    ...
    const canViewAllPatients = user?.role === 'admin' || user?.role === 'doctor' || user?.role === 'nurse';
    ...
    return (
        <div className="space-y-6">
                            ...
                            <CardContent className="p-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className={`text-sm mt-1 ${stat.change.startsWith('+') ? 'text-green-600' : stat.change === '0' ? 'text-slate-600' : 'text-red-600'}`}>
                                            {stat.change !== '0' && stat.change.startsWith('+') && '+'}{stat.change} today
                                        </p>
                                    </div>
                ...
                <TabsContent value="vitals" className="space-y-6">
                        ...
                        <CardContent className="p-6">
                            <div className="flex flex-col space-y-4">
                                <div className="relative">
                                    ...
                                    <Input
                                        placeholder={user?.role === 'patient' ? "Search your vital signs records..." : "Search patients or vital signs..."}
                                        value={searchTerm}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Latest vital signs measurements ({getFilteredData(mockVitalSigns).length} records)
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-4">
                                {getFilteredData(mockVitalSigns).map((vital) => (
                                    <div key={vital.id} className="p-4 border border-slate-200 rounded-lg">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                            ...
                                            <div className="bg-slate-50 p-3 rounded">
                                                ...
                                                <p className="mt-1">{vital.bloodPressureSystolic}/{vital.bloodPressureDiastolic}</p>
                                            </div>
                                            <div className="bg-slate-50 p-3 rounded">
                                                <div className="flex items-center space-x-2">
                                    ...
                                    <div key={diagnosis.id} className="p-4 border border-slate-200 rounded-lg">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-500">
                                                        Diagnosed
                                                        by {diagnosis.diagnosedBy} on {new Date(diagnosis.diagnosedDate).toLocaleDateString()}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                        ...
                                        {diagnosis.notes && (
                                            <div className="p-3 bg-slate-50 rounded">
                                                <Label className="text-sm font-medium text-slate-700">Clinical
                                                    Notes</Label>
                ...
                <TabsContent value="treatments" className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Active and completed treatment plans ({mockTreatmentPlans.length} total)
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-6">
                                {getFilteredData(mockTreatmentPlans).map((plan) => (
                                    <div key={plan.id} className="p-6 border border-slate-200 rounded-lg">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                <Label className="text-sm font-medium text-slate-700 mb-2 block">Treatment
                                                    Goals</Label>
                                            ...
                                            <div>
                                                <Label
                                                    className="text-sm font-medium text-slate-700 mb-2 block">Interventions</Label>
                                                <ul className="space-y-2">
                                        ...
                                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                <Label
                                                    className="text-sm font-medium text-slate-700 mb-2 block">Medications</Label>
                                                <ul className="space-y-2">
                                            ...
                                            <div>
                                                <Label
                                                    className="text-sm font-medium text-slate-700 mb-2 block">Timeline</Label>
                                        ...
                                        <div className="mt-4 p-3 bg-slate-50 rounded">
                                            <Label className="text-sm font-medium text-slate-700">Follow-up
                                                Instructions</Label>
            ...
            {/* Detail Modal */}
            <Dialog open={selectedModal.isOpen}
                    onOpenChange={() => setSelectedModal({isOpen: false, type: null, data: null})}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    {selectedModal.data && selectedModal.type === 'vitals' && (() => {
                    ...
                    {selectedModal.data && selectedModal.type === 'lab' && (() => {
                    ...
                    {selectedModal.data && selectedModal.type === 'diagnosis' && (() => {
                    ...
                    {selectedModal.data && selectedModal.type === 'treatment' && (() => {
