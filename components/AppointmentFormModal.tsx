...
import {AlertCircle, Calendar as CalendarIcon, Clock, Save, Stethoscope, User} from 'lucide-react';
            ...
            <DialogContent className="w-[1200px] h-[500px] max-w-[95vw] max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                    <DialogTitle className="flex items-center space-x-2">
                        ...
                        <span>
              {mode === 'schedule' ? 'Schedule New Appointment' :
                  mode === 'reschedule' ? 'Reschedule Appointment' :
                      'Appointment Details'}
            </span>
                    </DialogTitle>
                    <DialogDescription>
                        {mode === 'schedule' ? 'Create a new appointment for a patient' :
                            mode === 'reschedule' ? 'Modify appointment date and time' :
                        ...
                        <div className="space-y-4">
                                ...
                                <CardContent className="space-y-4">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div>
                                                <p className="font-medium">
                                                    {(() => {
                                                        const patient = Array.isArray(patients) ? patients.find(p => p.id === appointmentData.patientId) : null;
                                                        return patient ? `${patient.firstName} ${patient.lastName}` : 'Unknown Patient';
                                                    })()}
                                                </p>
                                                <p className="text-sm text-muted-foreground">
                                                    {Array.isArray(patients) ? patients.find(p => p.id === appointmentData.patientId)?.email : ''}
                                                </p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {appointmentData.doctorId && (
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div>
                                                <p className="font-medium">
                                                    {Array.isArray(doctors) ? doctors.find(d => d.id === appointmentData.doctorId)?.name : ''}
                                                </p>
                                                <p className="text-sm text-muted-foreground">
                                                    {Array.isArray(doctors) ? (doctors.find(d => d.id === appointmentData.doctorId)?.specialization || doctors.find(d => d.id === appointmentData.doctorId)?.department) : ''}
                                                </p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {selectedDate && appointmentData.appointmentTime && (
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    {appointmentData.appointmentTime} ({appointmentData.durationMinutes} minutes)
                                                </p>
                                            </div>
                                        </div>
                                    )}
                            ...
                            {appointmentData.notes && (
                                <Card>
                                    ...
                                    <CardContent>
                                        <p className="text-sm text-muted-foreground">{appointmentData.notes}</p>
                                    </CardContent>
                                </Card>
                            )}
                        ...
                        <Button onClick={handleSubmit} disabled={isSubmitting}>
                            {isSubmitting ? (
                                ...
                                <>
                                    ...
                                    {mode === 'schedule' ? 'Schedule Appointment' : 'Update Appointment'}
                                </>
                            )}
                        </Button>
                    )}
