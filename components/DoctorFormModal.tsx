...
export function DoctorFormModal({ isOpen, onClose, doctorId, mode }: DoctorFormModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const apiData = {
                ...
                ...(mode === 'add' && { passwordHash: doctorData.passwordHash })
            };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="w-[600px] max-w-[95vw] max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <DialogTitle className="flex items-center space-x-2">
                        ...
                        <span>
                            {mode === 'add' ? 'Add New Doctor' :
                             mode === 'edit' ? 'Edit Doctor Information' :
                             'Doctor Information'}
                        </span>
                    </DialogTitle>
                    <DialogDescription>
                        {mode === 'add' ? 'Enter doctor details and professional information' :
                         mode === 'edit' ? 'Update doctor information and contact details' :
                        ...
                        <>
                                        ...
                                        <div className="flex items-start space-x-4">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center space-x-2 mb-2">
                                                    <h3 className="text-xl font-semibold text-foreground">{doctorData.name}</h3>
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Doctor details and contact information</CardDescription>
                                </CardHeader>
                ...
                <div className="flex justify-between items-center pt-6 border-t border-border bg-gradient-to-r from-muted/50 to-emerald-50/30 -mx-6 px-6 pb-6 mt-6 rounded-b-xl">
                    <Button variant="outline" onClick={onClose} className="bg-background/80 backdrop-blur-sm border-border hover:bg-muted">
                        Cancel
                    </Button>
                    {!isReadOnly && (
                        <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-lg">
                            {isSubmitting ? (
                                <>
