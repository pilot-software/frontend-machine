                    ...
                    <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-4">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                ...
                                >
                                    <Avatar
                                        className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 transition-all duration-300 group-hover:animate-wave overflow-hidden">
                                        <AvatarImage src={user.avatar} alt={user.name}
                                                     className="transition-all duration-200 group-hover:brightness-110"/>
                                        <AvatarFallback
                                            className="transition-all duration-200 group-hover:bg-blue-500 group-hover:text-white text-xs">{getInitials(user.name)}</AvatarFallback>
                                    </Avatar>
                                    <div className="text-left hidden lg:block">
                                        ...
                                        <Badge variant="secondary"
                                               className={`${getRoleColor(user.role)} transition-all duration-200 group-hover:scale-105 text-xs`}>
                                            <RoleIcon
                                                className="h-3 w-3 mr-1 transition-transform duration-200 group-hover:rotate-12"/>
                            ...
                            <DropdownMenuContent align="end"
                                ...
                                <DropdownMenuLabel>
                                    <div className="flex items-center space-x-3 animate-in fade-in-50 duration-300">
                                        <Avatar
                                            className="h-10 w-10 ring-2 ring-blue-500/20 transition-all duration-300 hover:ring-blue-500/40 hover:scale-105">
                                            <AvatarImage src={user.avatar} alt={user.name}
                                                         className="transition-all duration-300 hover:brightness-110"/>
                                            <AvatarFallback
                                                className="bg-gradient-to-br from-blue-500 to-purple-600 text-white font-semibold">{getInitials(user.name)}</AvatarFallback>
                                        </Avatar>
                                ...
                                <DropdownMenuItem onClick={() => router.push("/settings")}
                                                  className="cursor-pointer group">
                                    <Settings
                                        className="h-4 w-4 mr-2 transition-transform duration-200 group-hover:rotate-90"/>
                                    Settings
                                </DropdownMenuItem>
                                <DropdownMenuSeparator/>
                                <DropdownMenuItem onClick={logout} className="text-destructive cursor-pointer group">
                                    <LogOut
                                        className="h-4 w-4 mr-2 transition-transform duration-200 group-hover:-translate-x-1"/>
                                    {text.buttons.logout}
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
