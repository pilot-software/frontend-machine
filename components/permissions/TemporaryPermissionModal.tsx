      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <RadioGroup value={permissionType} onValueChange={(value: 'group' | 'individual') => setPermissionType(value)} className="space-y-3">
              <div className="flex items-center space-x-3">
                <RadioGroupItem value="group" id="group" className="border-2 border-gray-400 data-[state=checked]:border-blue-600 data-[state=checked]:bg-blue-50" />
                <Label htmlFor="group" className="cursor-pointer font-medium">Permission Group</Label>
              </div>
              <div className="flex items-center space-x-3">
                <RadioGroupItem value="individual" id="individual" className="border-2 border-gray-400 data-[state=checked]:border-blue-600 data-[state=checked]:bg-blue-50" />
                <Label htmlFor="individual" className="cursor-pointer font-medium">Individual Permission</Label>
              </div>
            </RadioGroup>
          </div>
                ...
                <SelectContent>
                  {loading ? (
                    ...
                    permissionGroups.map(group => (
                      <SelectItem key={group.value || group.name} value={group.value || group.name}>
                        <div>
                ...
                <SelectContent>
                  {loading ? (
                    ...
                    individualPermissions.map(permission => (
                      <SelectItem key={permission.name || permission} value={permission.name || permission}>
                        <div>
                          <div className="font-medium">{(permission.name || permission).replace(/_/g, ' ')}</div>
