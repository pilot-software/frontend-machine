...
export function usePatientForm(isOpen: boolean, patientId?: string, mode?: string) {
  const { refetch } = useAppData();
  const reduxDoctors = useAppSelector((state) => state.app.doctors || []);
  
  const [patientData, setPatientData] = useState<PatientFormData>(INITIAL_FORM_DATA);
  const [selectedPatient, setSelectedPatient] = useState<any>(null);
  const [medicalData, setMedicalData] = useState<MedicalData | null>(null);
  const [doctors, setDoctors] = useState<any[]>([]);
  const [loading, setLoading] = useState({ selectedPatient: false, updating: false });
  const [loadingMedical, setLoadingMedical] = useState(false);
  const [error, setError] = useState<{ selectedPatient: string | null }>({ selectedPatient: null });
  ...
  useEffect(() => {
    const loadData = async () => {
      if (isOpen) {
        try {
          const doctorUsers = reduxDoctors?.length ? reduxDoctors : await userService.getUsersByRole("DOCTOR");
        ...
        if (patientId && (mode === "edit" || mode === "view")) {
          ...
          try {
            ...
            setError(prev => ({ ...prev, selectedPatient: error instanceof Error ? error.message : 'Unknown error' }));
          } finally {
  ...
  useEffect(() => {
    if (selectedPatient) {
      const raw = (selectedPatient as any).bloodType ?? (selectedPatient as any).blood_type ?? (selectedPatient as any).btype ?? "";
      const bloodType = raw && typeof raw === "string" ? (raw.includes("_") ? raw : BLOOD_TYPE_MAP[raw] ?? raw) : "";
      
      const doctorId = (selectedPatient as any).assignedDoctor ?? (selectedPatient as any).assignedDoctorId ?? 
                       (selectedPatient as any).primaryDoctorId ?? (selectedPatient as any).doctorId;
      const assignedDoctor = doctorId ? (typeof doctorId === "string" ? doctorId : doctorId.id || "") : "";
  ...
  const handleSubmit = useCallback(async (onClose: () => void) => {
    ...
    try {
      ...
      alert(`Failed to save patient: ${error instanceof Error ? error.message : "Unknown error occurred"}`);
    } finally {
