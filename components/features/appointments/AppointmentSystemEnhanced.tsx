...
export function AppointmentSystemEnhanced() {
    ...
    const [selectedType, setSelectedType] = useState<'all' | 'direct' | 'phone' | 'video'>('all');
    ...
    const todayAppointments = getFilteredAppointments().filter(apt => apt.date === '2024-08-01');
    const upcomingAppointments = getFilteredAppointments().filter(apt => apt.date > '2024-08-01');
    const completedAppointments = getFilteredAppointments().filter(apt => apt.status === 'completed');
    ...
    const todayStats = [
        ...
        {
            label: 'Direct Visits',
            value: todayAppointments.filter(apt => apt.type === 'direct').length.toString(),
            icon: User,
        ...
        {
            label: 'Phone Calls',
            value: todayAppointments.filter(apt => apt.type === 'phone').length.toString(),
            icon: Phone,
        ...
        {
            label: 'Video Calls',
            value: todayAppointments.filter(apt => apt.type === 'video').length.toString(),
            icon: Video,
    ...
    const canScheduleAppointments = user?.role === ROLES.ADMIN || user?.role === ROLES.NURSE || user?.role === ROLES.DOCTOR;
                            ...
                            <CardContent className="p-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className={`mt-1 ${stat.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <TabsContent value="today" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2">
                            <Card>
                                <CardHeader>
                                    <CardTitle>Today&apos;s Appointments - {new Date().toLocaleDateString()}</CardTitle>
                                    <CardDescription>
                                        ...
                                        {todayAppointments.map((appointment) => {
                                            ...
                                            return (
                                                <div key={appointment.id}
                                                     className={`p-4 border rounded-lg ${appointment.priority === 'urgent' ? 'border-red-300 bg-red-50' : 'border-slate-200'}`}>
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-4">
                                                            ...
                                                            <div className="flex-1">
                                                                ...
                                                                <p className="mb-1">
                                                                    {user?.role !== 'doctor' && `${appointment.doctorName} • `}
                                                        ...
                                                        <div className="flex items-center space-x-2">
                                                            {appointment.type === 'phone' && appointment.status === 'confirmed' && (
                                                                <Button variant="outline" size="sm">
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-2 max-h-96 overflow-y-auto">
                                        {getTimeSlots().slice(0, 20).map((slot) => (
                                            <div
                                                key={slot.time}
                ...
                <TabsContent value="upcoming" className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Appointments scheduled for the next 30 days ({upcomingAppointments.length} total)
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                        ...
                                        <TableRow key={appointment.id}>
                                            <TableCell>
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-500">{formatTime(appointment.time)}</p>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex items-center space-x-2">
                                            ...
                                            <TableCell>
                                                <div className="flex items-center space-x-1">
                                                    {React.createElement(getTypeIcon(appointment.type), {className: "h-4 w-4"})}
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        <p>Calendar view appointments will be displayed here based on selected date.</p>
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                ...
                <TabsContent value="history">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Past appointments and their outcomes ({completedAppointments.length} completed)
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-4">
